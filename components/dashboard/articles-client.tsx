...
import { Newspaper, ExternalLink, Filter, Wand2, Square, CheckSquare, Calendar, Tag, RefreshCw } from "lucide-react"
...
export function ArticlesClient({ articles }: Props) {
  ...
  const filtered = useMemo(() => articles.filter(a => {
    ...
    return true
  }), [articles, filterSource, filterStatus, search])
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <div className="relative">
          <input
            ...
            className="h-9 pl-9 pr-3 text-sm rounded-lg border border-slate-200 dark:border-white/10 bg-white dark:bg-white/5 text-slate-900 dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-cyan-500 w-56"
          />
      ...
      {filtered.length === 0 ? (
        <div className="bg-white dark:bg-white/5 border border-slate-200 dark:border-white/10 rounded-2xl p-12 text-center">
          ...
          <p className="text-xs text-slate-400 dark:text-slate-500 mt-1">
            {articles.length === 0 ? "Ajoutez des sources RSS pour commencer à collecter des articles." : "Modifiez vos filtres pour voir plus d'articles."}
          </p>
        </div>
      ) : (
              ...
              >
                ...
                <div className="p-4 pr-10">
                  ...
                  <div className="flex items-center gap-2 mb-2.5">
                    ...
                    <span className={`ml-auto text-xs px-1.5 py-0.5 rounded-full font-medium ${statusColors[article.status] ?? statusColors.pending}`}>
                  ...
                  <div className="flex items-center justify-between mt-auto pt-2 border-t border-slate-100 dark:border-white/5">
                    {article.author && (
                      <span className="text-xs text-slate-400 truncate max-w-[120px]">{article.author}</span>
                    )}
                    <a
                      href={article.url}
                      target="_blank"
                      ...
                      className="ml-auto text-slate-400 hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors"
                    >
